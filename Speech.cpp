...
#include "nrf.h"
#define MQTT_SPEECH_HAS_PDM 1
#else
#define MQTT_SPEECH_HAS_PDM 0
#endif
...
extern "C" void PDM_IRQHandler(void) {
    if (NRF_PDM->EVENTS_END) {
        NRF_PDM->EVENTS_END = 0;
        if (MQTTSpeech::running) {
            MQTTSpeech::completedSampleBuffer = MQTTSpeech::currentSampleBuffer;
            MQTTSpeech::currentSampleBuffer = MQTTSpeech::currentSampleBuffer == 0 ? 1 : 0;
            NRF_PDM->SAMPLE.PTR = (uint32_t)(MQTTSpeech::currentSampleBuffer == 0
                ? MQTTSpeech::sampleA
                : MQTTSpeech::sampleB);
            NRF_PDM->SAMPLE.MAXCNT = MQTTSpeech::DMA_SAMPLES;
        }
    }
}
#endif
